...
这是一个简单的多智能体环境示例，展示如何实现符合MARL框架要求的环境接口。
"""
...
if __name__ == "__main__":
    ...
    # 创建环境
    env = SimpleMultiAgentEnv(cfg, rank=0, num_envs=4)
    
    # 测试reset
    print("Testing reset...")
    obs_dict, infos = env.reset()
    print(f"Obs shapes: { {k: v.shape for k, v in obs_dict.items()} }")
    
    # 测试step
    print("\nTesting step...")
    actions = {
        agent_id: torch.randn(4, 8) 
        for agent_id in ["agent_0", "agent_1"]
    }
    obs, rewards, dones, truncations, infos = env.step(actions)
    print(f"Reward shapes: { {k: v.shape for k, v in rewards.items()} }")
    print(f"Done shapes: { {k: v.shape for k, v in dones.items()} }")
    
    # 测试chunk_step
    print("\nTesting chunk_step...")
    chunk_actions = {
        agent_id: torch.randn(4, 5, 8)  # [num_envs, num_chunks, action_dim]
        for agent_id in ["agent_0", "agent_1"]
    }
    obs, rewards, dones, truncations, infos = env.chunk_step(chunk_actions)
    print(f"Chunk obs shapes: { {k: v.shape for k, v in obs.items()} }")
    print(f"Chunk reward shapes: { {k: v.shape for k, v in rewards.items()} }")
