...
"""
测试SimpleMARLEnv环境（独立测试脚本）

这个脚本可以在examples/marl目录下直接运行，用于测试环境。
"""
...
def test_simple_marl_env():
    ...
    try:
        ...
        print(f"   ✓ reset成功")
        print(f"   观察形状: { {k: tuple(v.shape) for k, v in obs_dict.items()} }")
        print(f"   观察设备: { {k: str(v.device) for k, v in obs_dict.items()} }")
    ...
    try:
        ...
        print(f"   ✓ step成功")
        print(f"   观察形状: { {k: tuple(v.shape) for k, v in obs.items()} }")
        print(f"   奖励形状: { {k: tuple(v.shape) for k, v in rewards.items()} }")
        print(f"   Done形状: { {k: tuple(v.shape) for k, v in dones.items()} }")
        print(f"   Truncation形状: { {k: tuple(v.shape) for k, v in truncations.items()} }")
    ...
    try:
        ...
        print(f"   观察形状: { {k: tuple(v.shape) for k, v in obs.items()} }")
        print(f"   奖励形状: { {k: tuple(v.shape) for k, v in rewards.items()} }")
        print(f"   Termination形状: { {k: tuple(v.shape) for k, v in terminations.items()} }")
        print(f"   Truncation形状: { {k: tuple(v.shape) for k, v in truncations.items()} }")
